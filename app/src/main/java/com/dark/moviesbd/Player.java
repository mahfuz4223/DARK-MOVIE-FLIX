...
import com.dark.moviesbd.db.resume_content.ResumeContent;
import com.dark.moviesbd.db.resume_content.ResumeContentDatabase;
import com.dark.moviesbd.dialog.TrackSelectionDialog;
import com.dark.moviesbd.list.MultiqualityList;
import com.dark.moviesbd.list.YTStreamList;
import com.dark.moviesbd.utils.HelperUtils;
import com.dark.moviesbd.utils.Utils;
import com.dark.moviesbd.utils.Yts;
import com.dark.moviesbd.utils.stream.Cinematic;
import com.dark.moviesbd.utils.stream.Facebook;
import com.dark.moviesbd.utils.stream.GoFile;
import com.dark.moviesbd.utils.stream.Tubitv;
import com.dark.moviesbd.utils.stream.Vimeo;
import com.dark.moviesbd.utils.stream.Yandex;
...
public class Player extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        imgBrightness.setOnClickListener(view->{
            if (brightness.getVisibility() == View.VISIBLE) {
                brightness.setVisibility(View.GONE);
            } else if (brightness.getVisibility() == View.GONE) {
                brightness.setVisibility(View.VISIBLE);
            }
        });

        brightness.setMax(getMaxBrightness(this, 1000));
        ...
        imgVolume.setOnClickListener(view->{
            ...
            } else if (volume.getVisibility() == View.GONE) {
    ...
    void Prepare_Source(String source, String url) {
        ...
        if (source.equals("M3u8")) {
            ...
            HlsMediaSource mediaSource = new HlsMediaSource.Factory(dataSourceFactory).createMediaSource(MediaItem.fromUri(Uri.parse(AppConfig.url + "/api/fetch/streamsb/get_ss.php?url=" + url)));
            initializePlayer(mediaSource);
    ...
    void initializePlayer(MediaSource mediaSource) {
        ...
        /*Format textFormat2 = Format.createTextSampleFormat("2",  MimeTypes.APPLICATION_SUBRIP,  Format.NO_VALUE, "bn");
        MediaSource textMediaSource2 = new SingleSampleMediaSource.Factory(dataSourceFactory)
                .createMediaSource(Uri.parse("http://www.storiesinflight.com/js_videosub/jellies.srt"), textFormat2, C.TIME_UNSET);
        n_mediaSource = new MergingMediaSource(n_mediaSource, textMediaSource2);*/
            ...
            if(!response.equals("No Data Avaliable")) {
                ...
                for (JsonElement rootElement : jsonArray) {
                    ...
                    if(rootObject.get("mime_type").getAsString().equals("WebVTT")) {
                        textFormat = Format.createTextSampleFormat(String.valueOf(rootObject.get("id").getAsInt()), MimeTypes.TEXT_VTT,  Format.NO_VALUE, rootObject.get("language").getAsString());
                    } else if(rootObject.get("mime_type").getAsString().equals("TTML") || rootObject.get("mime_type").getAsString().equals("SMPTE-TT")) {
                        textFormat = Format.createTextSampleFormat(String.valueOf(rootObject.get("id").getAsInt()), MimeTypes.APPLICATION_TTML,  Format.NO_VALUE, rootObject.get("language").getAsString());
                    } else if(rootObject.get("mime_type").getAsString().equals("SubRip")) {
                        textFormat = Format.createTextSampleFormat(String.valueOf(rootObject.get("id").getAsInt()), MimeTypes.APPLICATION_SUBRIP,  Format.NO_VALUE, rootObject.get("language").getAsString());
                        ...
                        textFormat = Format.createTextSampleFormat(String.valueOf(rootObject.get("id").getAsInt()), MimeTypes.TEXT_SSA,  Format.NO_VALUE, rootObject.get("language").getAsString());
                    } else {
                        textFormat = Format.createTextSampleFormat(String.valueOf(rootObject.get("id").getAsInt()), MimeTypes.APPLICATION_SUBRIP,  Format.NO_VALUE, rootObject.get("language").getAsString());
                    }
                    ...
                    MediaSource textMediaSource = new SingleSampleMediaSource.Factory(dataSourceFactory)
                            .createMediaSource(Uri.parse(rootObject.get("subtitle_url").getAsString()), textFormat, C.TIME_UNSET);
